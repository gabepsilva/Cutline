...
function parseInventory() {
  ...
  return issues.map((row) => {
    ...
    const dataReq = parseDeps(row['Data required'] || '');
    return {
      id,
      ms,
...
function typeOf(it) {
  ...
  const svelte = it.files.some((f) => f.endsWith('.svelte') || f.endsWith('/*.svelte') || f.includes('*.svelte'));
  ...
  if (it.files.every((f) => f.endsWith('.ts') || f.endsWith('.css') || f.includes('*.ts'))) return 'util';
  return 'component';
}
...
function header(it) {
  ...
  const blockedBy =
    it.deps.pids.length || it.deps.notes.length
      ? [...it.deps.pids.map((p) => (pidMap[p] ? `${p} (#${pidMap[p]})` : p)), ...it.deps.notes].join(', ')
      : '—';
  const readFirst =
  ...
  return [
    ...
    it.lines ? `> **Design source:** \`${DESIGN}\` (lines ${it.lines})` : `> **Design source:** \`${DESIGN}\``,
...
function ensureLabel(name, color = 'ededed', desc = '') {
  if (DRY) return;
  try {
    execFileSync('gh', ['label', 'create', name, '--color', color, '--description', desc, '--force', '--repo', REPO], {
      stdio: 'ignore'
    });
  } catch {
    /* ignore */
...
function existingPidIssue(id) {
  ...
  try {
    const out = sh('gh', ['issue', 'list', '--repo', REPO, '--label', `pid:${id}`, '--state', 'all', '--json', 'number', '--jq', '.[0].number']);
...
function createIssue(it) {
  ...
  const a = ['issue', 'create', '--repo', REPO, '--title', title, '--body', body, '--milestone', milestoneTitles[it.ms]];
...
if (has('--all')) selected = inventory;
  ...
  const want = val('--pids').split(',').map((s) => s.trim());
