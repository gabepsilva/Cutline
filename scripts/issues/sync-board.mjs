...
try {
	...
	console.error(
		"ERROR: gh is missing the 'project' scope. Run: gh auth refresh -s project,read:project"
	);
	process.exit(1);
}
...
try {
	...
	gql(
		'mutation($p:ID!,$r:ID!){linkProjectV2ToRepository(input:{projectId:$p,repositoryId:$r}){repository{name}}}',
		{
			p: proj.id,
			r: repoId
		}
...
if (options.map((o) => o.name).join(',') !== wanted) {
	const opts = COLUMNS.map(
		([name, color, description]) => `{name:"${name}",color:${color},description:"${description}"}`
	).join(',');
	options = gql(
...
console.log(
	`board synced: ${added} added, ${set} status set. https://github.com/users/${OWNER}/projects/${proj.number}`
);
